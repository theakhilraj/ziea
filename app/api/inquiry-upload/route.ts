...
// PUBLIC design-inquiry reference/moodboard-image upload. Unlike
// app/api/upload/route.ts (admin-only, arbitrary folder), this endpoint is
// callable by unauthenticated customers, so it is deliberately hardened: a tight
// image allow-list, a small size cap, a per-IP rate limit, and a HARD-CODED
// destination folder (callers can never choose where their bytes land). Faithful
// clone of app/api/consultation-upload/route.ts.
...
const INQUIRY_FOLDER = 'inquiries';
...
//
// RATE_LIMIT_MAX is 20 here — higher than consultation's 5 — because a design
// inquiry can be a GROUP moodboard of up to 10 images (each a separate POST),
// plus retries, so a legitimate single session issues many more uploads.
...
export async function POST(request: Request) {
  try {
    ...
    // guard so the public can never traverse out of the inquiries folder.
    const folder = safeFolder(INQUIRY_FOLDER);
